      ...
      <tbody>
		...
		<table width="100%" class="table table-striped table-bordered">
			...
			<tbody>
				<?php $total=0; $bobots1z = $bobotObj->readAll2(); while ($row1 = $bobots1z->fetch(PDO::FETCH_ASSOC)): ?>
					<tr>
						...
						<th class="info"><?=number_format($sumRow[$row1["id_kriteria"]], 2, '.', ',')?></th>
						<th class="success"><?=number_format($row1["bobot_kriteria"], 2, '.', ',');?></th>
						<?php $jumlah = $sumRow[$row1["id_kriteria"]] + $row1["bobot_kriteria"]; ?>
						<th class="warning"><?=number_format($jumlah, 2, '.', ',');?></th>
						<?php $total += $jumlah; ?>
					</tr>
				<?php endwhile; ?>
			</tbody>
			<tfoot>
				<tr class="danger">
					...
					<th><?php $rata = $total/$count; echo number_format($rata, 2, '.', ','); ?></th>
				</tr>
			</tfoot>
		</table>
		...
		<table width="100%" class="table table-striped table-bordered">
			<tbody>
				...
				<tr>
					<th>IR (Ukuran Matriks 3 maka IR = 0.90</th>
					<td><?php echo $ir = $bobotObj->getIr($count); ?></td>
				</tr>
				<tr>
					<th>CI</th>
					<td><?php $ci = ($rata-$count)/($count-1); echo number_format($ci, 2, '.', ',');?></td>
				</tr>
				<tr>
					<th>CR</th>
